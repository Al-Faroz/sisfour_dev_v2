...
--
-- SAFETY:
-- - Schema/seed/RBAC/menu delta only.
-- - DO NOT create table migrations.
-- - DO NOT copy localhost UAT data (CKG/kunjungan/user/pegawai).
-- - DO NOT alter intentional hosting FK konseling_bk.id_guru_bk
--   (ON DELETE SET NULL ON UPDATE CASCADE).
-- - Script is written idempotently for expected hosting baseline.
...
-- Hosting post-execution verification.
-- Expected operational tables remain empty because localhost UAT data is not copied.
SELECT 'uks_ckg' AS entity, COUNT(*) AS total FROM uks_ckg
UNION ALL
SELECT 'uks_kunjungan', COUNT(*) FROM uks_kunjungan
UNION ALL
SELECT 'uks_kunjungan_tindakan', COUNT(*) FROM uks_kunjungan_tindakan;

-- Expected canonical reference counts: Keluhan 17, Tindakan 5, Hasil 4.
SELECT 'uks_ref_keluhan' AS entity, COUNT(*) AS total FROM uks_ref_keluhan
UNION ALL
SELECT 'uks_ref_tindakan', COUNT(*) FROM uks_ref_tindakan
UNION ALL
SELECT 'uks_ref_hasil', COUNT(*) FROM uks_ref_hasil;

-- Local UAT-only reference must never be present on hosting.
SELECT id, nama, status_aktif
FROM uks_ref_tindakan
WHERE nama = 'Lari-Lari';

-- Hosting must continue without local-only migrations table.
SHOW TABLES LIKE 'migrations';

-- Preserve the intentional G3.3.1 hosting FK behavior; audit read-only.
