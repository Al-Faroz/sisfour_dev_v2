-- G3.6B PTSP — HOSTING
-- Prepared from fresh pre-SQL hosting dump: u473908839_sisfour2026 (14).sql
-- Dump baseline audit: 41 tables / 55 permissions / 183 role_permissions / 46 menus / 156 role_menus.
-- PTSP baseline: 0 tables / 0 permissions / 0 menus / 0 operational rows.
-- Source baseline main: 90acc7f94fee391a5a7fbad2395e3f16571fe921
-- Feature branch target: feat/g3-6b-ptsp-20260919
-- IMPORTANT:
--   1) SQL ini hanya untuk hosting setelah approval eksekusi eksplisit.
--   2) Tidak membawa data UAT localhost.
--   3) Tidak mengubah schema/data UKS atau Konseling.
--   4) setting ptsp_layanan_auto_print dan ptsp_layanan_auto_download_pdf tidak di-seed;
--      default aplikasi keduanya OFF sampai disimpan melalui Settings -> Setting Sistem.
-- Expected post-SQL counts:
--   tables=45, permissions=66, role_permissions=223, menus=50, role_menus=173.
...
SELECT COUNT(*) AS ptsp_permission_count
FROM permissions
WHERE permission_key LIKE 'ptsp_%';

SELECT COUNT(*) AS ptsp_menu_count
FROM menus
WHERE nama_menu='PTSP' OR link IN ('ptsp/layanan','ptsp/polling','ptsp/pengaduan');

SELECT setting_key, setting_value, type
FROM setting_sistem
WHERE setting_key IN ('ptsp_layanan_auto_print','ptsp_layanan_auto_download_pdf')
ORDER BY setting_key;

SELECT rc.CONSTRAINT_NAME, rc.DELETE_RULE, rc.UPDATE_RULE
FROM information_schema.REFERENTIAL_CONSTRAINTS rc
WHERE rc.CONSTRAINT_SCHEMA = DATABASE()
  AND rc.CONSTRAINT_NAME = 'fk_konseling_guru_bk';
