...
-- Tanggal: 2026-09-17
-- Canonical record setelah dump hosting aktual diaudit dan delta dieksekusi PASS oleh user.
--
-- Tujuan:
...
-- 4) backfill hanya bila period dapat ditentukan tanpa menebak;
-- 5) collation follow-up dinormalisasi ke utf8mb4_general_ci agar selaras schema canonical.
--
-- CATATAN RERUN:
-- Kolom/index memakai IF NOT EXISTS. FK bernama adalah delta satu kali;
...
ALTER TABLE `u473908839_sisfour2026`.`catatan_prestasi`
  ADD COLUMN IF NOT EXISTS `id_tahun` INT(10) UNSIGNED NULL AFTER `id`;
...
-- Record ambigu tetap NULL; tidak menebak period dari tanggal.
UPDATE `u473908839_sisfour2026`.`catatan_kasus` ck
SET ck.`id_tahun` = (
  SELECT MIN(ak.`id_tahun`)
  FROM `u473908839_sisfour2026`.`anggota_kelas` ak
  WHERE ak.`id_siswa` = ck.`id_siswa`
)
WHERE ck.`id_tahun` IS NULL
  AND 1 = (
    SELECT COUNT(DISTINCT ak2.`id_tahun`)
    FROM `u473908839_sisfour2026`.`anggota_kelas` ak2
    WHERE ak2.`id_siswa` = ck.`id_siswa`
  );

UPDATE `u473908839_sisfour2026`.`catatan_prestasi` cp
SET cp.`id_tahun` = (
  SELECT MIN(ak.`id_tahun`)
  FROM `u473908839_sisfour2026`.`anggota_kelas` ak
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `u473908839_sisfour2026`.`tindak_lanjut_konseling_bk` (
  ...
  CONSTRAINT `fk_tlk_konseling`
    ...
    REFERENCES `u473908839_sisfour2026`.`konseling_bk` (`id`)
    ON DELETE RESTRICT ON UPDATE CASCADE,
...
-- Jika tabel sempat tercipta mengikuti default collation hosting, normalisasi ke canonical.
ALTER TABLE `u473908839_sisfour2026`.`tindak_lanjut_konseling_bk`
...
SHOW CREATE TABLE `u473908839_sisfour2026`.`catatan_prestasi`;
SHOW CREATE TABLE `u473908839_sisfour2026`.`tindak_lanjut_konseling_bk`;

SELECT COUNT(*) AS `catatan_kasus_tahun_belum_terpetakan`
FROM `u473908839_sisfour2026`.`catatan_kasus`
WHERE `id_tahun` IS NULL;

SELECT COUNT(*) AS `prestasi_tahun_belum_terpetakan`
FROM `u473908839_sisfour2026`.`catatan_prestasi`
WHERE `id_tahun` IS NULL;

SELECT COUNT(*) AS `jumlah_tindak_lanjut_konseling`
FROM `u473908839_sisfour2026`.`tindak_lanjut_konseling_bk`;
