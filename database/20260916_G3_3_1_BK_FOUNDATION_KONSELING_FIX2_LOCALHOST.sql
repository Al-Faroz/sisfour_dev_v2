...
-- Tanggal: 2026-09-16
--
-- Direkonsiliasi terhadap dump localhost aktual 2026-09-16.
-- Script ini sengaja TIDAK bergantung pada database yang sedang dipilih di phpMyAdmin.
-- Semua tabel aplikasi dan lookup information_schema memakai schema eksplisit.
--
-- Sasaran:
...
-- id_guru_bk adalah metadata legacy optional; akun BK lokal menggunakan users.id_pegawai.
SET @has_id_guru_bk := (
  SELECT COUNT(*)
...
SET @sql := IF(
  @has_id_guru_bk > 0,
  'ALTER TABLE `sisfour_dev_v2`.`konseling_bk` MODIFY `id_guru_bk` int(10) UNSIGNED DEFAULT NULL',
  'SELECT 1'
);
...
SET @sql := IF(
  ...
  IF(
    @has_id_guru_bk > 0,
    'ALTER TABLE `sisfour_dev_v2`.`konseling_bk` ADD COLUMN `created_by` int(10) UNSIGNED DEFAULT NULL AFTER `id_guru_bk`',
    'ALTER TABLE `sisfour_dev_v2`.`konseling_bk` ADD COLUMN `created_by` int(10) UNSIGNED DEFAULT NULL AFTER `status`'
  ),
  'SELECT 1'
);
PREPARE stmt FROM @sql;
...
UPDATE `sisfour_dev_v2`.`konseling_bk`
...
SET @sql := IF(
  @has_created_idx = 0,
  'ALTER TABLE `sisfour_dev_v2`.`konseling_bk` ADD KEY `idx_konseling_created_by` (`created_by`)',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @has_created_fk := (
  ...
  WHERE CONSTRAINT_SCHEMA = 'sisfour_dev_v2'
    AND TABLE_NAME = 'konseling_bk'
...
SET @sql := IF(
  @has_created_fk = 0,
  'ALTER TABLE `sisfour_dev_v2`.`konseling_bk` ADD CONSTRAINT `fk_konseling_created_by` FOREIGN KEY (`created_by`) REFERENCES `sisfour_dev_v2`.`users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
-- role_permissions memakai UNIQUE(role,id_permission,scope), sehingga UPSERT scope saja
-- tidak cukup untuk menghapus scope lama. Normalisasi key Konseling secara deterministik.
DELETE rp
...
-- Sinkronkan label menu lama.
UPDATE `sisfour_dev_v2`.`menus`
SET `nama_menu` = 'Catatan Pelanggaran', `updated_at` = NOW()
WHERE `link` = 'bk/kasus';

-- Pastikan menu Konseling BK ada di parent BK yang sama dengan Catatan Pelanggaran.
SET @bk_parent_id := (
  SELECT `parent_id`
...
-- Verifikasi. Semua query memakai schema eksplisit agar aman walau phpMyAdmin
-- sedang berada pada information_schema atau database lain.
SELECT 'sisfour_dev_v2' AS `database_target`;

SELECT c.COLUMN_NAME, c.IS_NULLABLE, c.COLUMN_TYPE
FROM information_schema.COLUMNS c
WHERE c.TABLE_SCHEMA = 'sisfour_dev_v2'
...
-- BK adalah Pegawai pada database lokal aktual.
