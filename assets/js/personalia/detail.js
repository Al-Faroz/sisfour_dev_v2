(() => {
  ...
  const config = {
    pendidikan: {
      modal: 'modalPendidikan',
      tab: '#tabPendidikan',
      label: 'Pendidikan',
    },
    penugasan: {
      modal: 'modalPenugasan',
      tab: '#tabPenugasan',
      label: 'Penugasan',
    },
    pangkat: {
  ...
  const confirmDelete = async (category) => {
    const label =
      config[category]?.label || 'Personalia';
    ...
    if (window.Swal) {
      const result = await Swal.fire({
        ...
        title: `Hapus ${label}?`,
        text:
          'Record dan dokumen yang melekat akan dihapus. '
    ...
    return window.confirm(
      `Hapus record ${label} beserta dokumen yang melekat?`
    );
  };
  ...
  const formFor = (category) =>
    document.querySelector(
      `.personalia-form[data-category="${category}"]`
    );
  ...
  const resetForm = (category) => {
    ...
    if (
      category === 'dokumen'
      && form.elements.file_dokumen
    ) {
      form.elements.file_dokumen.required = true;

      form
        .querySelector('.new-file-required')
        ?.classList.remove('d-none');
    }

    updateModalTitle(category, false);
  };
  ...
  const fillForm = (
    ...
    updateModalTitle(category, true);
  };
  ...
  if (canEdit) {
    ...
    document
      .querySelectorAll('.btn-edit-record')
      .forEach((button) => {
        button.addEventListener(
          'click',
          () => {
            ...
            try {
              ...
              notify(
                'Data record tidak dapat dibaca.',
                true
              );
            }
          }
    ...
    document
      .querySelectorAll('.personalia-form')
      .forEach((form) => {
          ...
          async (event) => {
            ...
            try {
              ...
              if (
                ...
                await notify(
                  payload.message
                    || 'Data personalia gagal disimpan.',
                  true
                );
              ...
              await notify(
                payload.message
                  || 'Data personalia berhasil disimpan.'
              );
    ...
    document
      .querySelectorAll('.btn-delete-record')
      .forEach((button) => {
        button.addEventListener(
          'click',
          async () => {
            ...
            try {
              const response =
                await fetch(
                  `${endpointBase}/delete/${category}/${id}`,
                  {
                    method: 'DELETE',
                    headers: {
                      Accept: 'application/json',
              ...
              await notify(
                payload.message
                  || 'Data personalia berhasil dihapus.'
              );
