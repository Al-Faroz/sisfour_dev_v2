(() => {
  ...
  const mobileList = document.getElementById('guruRecycleMobileList');
  ...
  async function load() {
    try {
      ...
      if (mobileList) {
        ...
        </div>`).join('') || '<div class="list-group-item sisfour-mobile-state text-muted">Recycle Bin Guru kosong.</div>';
      }
