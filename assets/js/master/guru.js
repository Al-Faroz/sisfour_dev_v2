(() => {
  ...
  const ensurePager = () => {
    let container = document.getElementById('guruPager');
    if (container) return container;

    container = document.createElement('div');
    container.id = 'guruPager';
    ...
    tbody.innerHTML = rows.map((row, index) => {
      ...
      return `<tr data-row="${data}">
        ...
        <td class="text-nowrap">
          ...
          <a href="${base}/master/guru/personalia/${row.id}" class="btn btn-sm btn-outline-dark" title="Riwayat & Portofolio Guru"><i class="bx bx-folder-open"></i></a>${manageActions}
        </td>
      </tr>`;
      ...
      mobileList.innerHTML = rows.map((row) => {
        ...
        return `<div class="list-group-item py-3" data-row="${data}">
          ...
          <div class="sisfour-mobile-actions mt-3">
            ...
            <a href="${base}/master/guru/personalia/${row.id}" class="btn btn-sm btn-outline-dark sisfour-touch-target--compact">Personalia</a>
            ${manageActions}
          </div>
        </div>`;
      }).join('') || '<div class="list-group-item sisfour-mobile-state text-muted">Tidak ada data Guru.</div>';
    }
  ...
  async function load() {
    ...
    try {
      const response = await fetch(`${base}/master/guru?${params(true)}`, {
  ...
  document.getElementById('btnExportGuru')?.addEventListener('click', (event) => {
    event.preventDefault();
    window.location.href = `${base}/master/guru/export?${params(false)}`;
  });
