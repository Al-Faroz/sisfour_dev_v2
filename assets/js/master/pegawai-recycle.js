(() => {
  ...
  async function load() {
    try {
      const response = await fetch(`${base}/master/pegawai/recycle?format=json`, {
        headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
      });
      const payload = await response.json();
      if (!response.ok || payload.status !== 'success') {
        await show(payload.message || 'Gagal memuat Recycle Bin Pegawai.', true);
        return;
      }
  ...
  async function mutate(button, action, method) {
    ...
    try {
      const response = await fetch(`${base}/master/pegawai/${action}/${id}`, {
        method,
        headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
      });
      const payload = await response.json();
