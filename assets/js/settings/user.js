...
async function load() {
  try {
    const r = await fetch(
      `${base}/settings/user?${queryParams()}`,
      {
        headers: {
    ...
    body.innerHTML = (d.rows || []).map(row => {
      const secondary = (row.secondary_roles || [])
        .map(
          x =>
            `<span class="badge bg-label-secondary me-1">${esc(x)}</span>`
        )
        .join('') || '-';
    ...
    info.textContent =
      `${total ? offset + 1 : 0}-${Math.min(offset + limit, total)} dari ${total}`;

    document.getElementById('userPrev').disabled =
      offset <= 0;
...
function clearIdentityValues() {
  ['id_guru', 'id_pegawai', 'id_siswa'].forEach(
    name => {
...
async function openEdit(id) {
  ...
  form.elements.status_aktif.value =
    String(u.status_aktif ?? 1);
  ...
  document
    .querySelectorAll('.secondary-role')
    .forEach(cb => {
      cb.checked = (u.secondary_roles || [])
        .includes(cb.value);
    });
  ...
  if (type && identityId) {
    document.getElementById('identityResult').innerHTML =
      `<option value="${esc(identityId)}" selected>`
      + `${esc(label)}</option>`;
...
async function searchIdentity() {
  const type = document
    .getElementById('identityType')
    .value;
  const search = document
    .getElementById('identitySearch')
    .value
    .trim();
  const select = document
    .getElementById('identityResult');
  ...
  select.innerHTML =
    ...
    + (j.data.rows || []).map(
      row =>
        `<option value="${row.id}" data-used="${row.id_user || ''}">`
        + `${esc(row.kode || '-')} — ${esc(row.nama)}`
        + `${row.id_user ? ' [sudah terhubung]' : ''}`
        + '</option>'
    ).join('');
}
...
function bindRows() {
  ...
  document
    .querySelectorAll('.btn-reset-user')
    .forEach(
      btn => btn.addEventListener(
        'click',
        () => {
  ...
  document
    ...
    .forEach(
      btn => btn.addEventListener(
        ...
        async () => {
          if (!confirm(
            'Hapus user ini? Untuk akun yang masih memiliki relasi data, '
            + 'gunakan Nonaktif bila penghapusan ditolak.'
          )) {
            return;
          }

          const r = await fetch(
            `${base}/settings/user/delete/${btn.dataset.id}`,
            {
              method: 'DELETE',
              headers: {
                Accept: 'application/json',
...
document
  .getElementById('btnUserBaru')
  .addEventListener('click', openCreate);

document
  .getElementById('btnUserCari')
  .addEventListener(
    'click',
    () => {
      offset = 0;
...
document
  .getElementById('btnIdentitySearch')
  .addEventListener('click', searchIdentity);
...
document
  .getElementById('identityResult')
  .addEventListener('change', applyIdentity);
...
form.addEventListener('submit', async e => {
  ...
  modal.hide();
  show(
    j.message || 'User berhasil disimpan.',
    'success'
  );
...
resetForm.addEventListener('submit', async e => {
  ...
  resetModal.hide();
  show(
    j.message || 'Password berhasil direset.',
    'success'
  );
