                ...
                <div class="col-12 col-lg-4">
                    <div class="card h-100">
                        <div class="card-header d-flex justify-content-between align-items-start flex-wrap gap-2">
                            ...
                            <button class="btn btn-sm btn-primary sisfour-touch-target--compact btn-master-new" type="button" data-type="<?= esc($type) ?>">Tambah</button>
                        </div>
                        <div class="list-group list-group-flush" data-list="<?= esc($type) ?>">
                            <?php foreach (($refs[$type] ?? []) as $row): ?>
                                ...
                                <div class="list-group-item d-flex justify-content-between align-items-start flex-wrap gap-2<?= $isActive ? '' : ' bg-body-tertiary' ?>" data-json="<?= esc(rawurlencode(json_encode($row))) ?>">
                                    <div class="min-w-0 flex-grow-1">
                                        <div class="d-flex align-items-center flex-wrap gap-2 mb-1">
                                            <div class="fw-semibold sisfour-wrap-anywhere<?= $isActive ? '' : ' text-muted' ?>"><?= esc($row['nama']) ?></div>
                                    ...
                                    <div class="sisfour-mobile-actions">
                                        <button class="btn btn-sm btn-outline-primary sisfour-touch-target--compact btn-master-edit" type="button" data-type="<?= esc($type) ?>">Edit</button>
                                        <?php if ($isActive): ?>
                                            <button class="btn btn-sm btn-outline-danger sisfour-touch-target--compact btn-master-delete" type="button" data-type="<?= esc($type) ?>">Nonaktifkan</button>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
