...
<div class="card mb-4">
  ...
  <div class="table-responsive">
    <table class="table table-sm mb-0">
      ...
      <tbody><?php if(empty($widgets['ews_top'])): ?><tr><td colspan="2" class="text-center text-muted py-4">Tidak ada siswa EWS.</td></tr><?php else: foreach($widgets['ews_top'] as $row): ?><tr><td><?= esc($row['nama']) ?></td><td><span class="badge bg-label-danger"><?= (int)$row['total_alpha'] ?></span></td></tr><?php endforeach; endif; ?></tbody>
    </table>
  </div>
</div>

<div class="row g-3">
  <div class="col-lg-6"><div class="card h-100"><div class="card-header"><h5 class="mb-0">Pelanggaran Terbaru</h5></div><ul class="list-group list-group-flush"><?php if(empty($widgets['kasus_terbaru'])): ?><li class="list-group-item text-muted text-center py-4">Belum ada catatan pelanggaran.</li><?php else: foreach($widgets['kasus_terbaru'] as $row): ?><li class="list-group-item"><strong><?= esc($row['nama']) ?></strong> — <?= esc($row['nama_pelanggaran']) ?><div><span class="badge bg-label-<?= ($row['kategori']??'')==='Berat'?'danger':(($row['kategori']??'')==='Sedang'?'warning':'secondary') ?>"><?= esc($row['kategori']) ?></span> <small class="text-muted"><?= esc($row['tanggal']) ?></small></div></li><?php endforeach; endif; ?></ul></div></div>
