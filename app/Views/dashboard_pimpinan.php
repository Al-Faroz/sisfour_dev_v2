...
<div class="card mb-4">
  ...
  <ul class="list-group list-group-flush"><?php if(empty($widgets['prestasi_terbaru'])): ?><li class="list-group-item text-muted text-center py-4">Belum ada data.</li><?php else: foreach($widgets['prestasi_terbaru'] as $row): ?><li class="list-group-item"><strong><?= esc($row['nama']) ?></strong><div><?= esc($row['nama_prestasi']) ?></div><small class="text-muted"><?= esc($row['tanggal']) ?> · <?= esc($row['tingkat'] ?? '-') ?></small></li><?php endforeach; endif; ?></ul>
</div>
