...
<h4 class="mb-1">Dashboard Wali Kelas</h4>
<p class="text-muted mb-4">Status Wali Kelas terdeteksi otomatis dari mapping_wali_kelas.</p>
...
<div class="card mt-4">
  <div class="card-header"><h5 class="mb-0">Ringkasan Kelas Diampu</h5></div>
  <div class="table-responsive text-nowrap">
    <table class="table">
      <thead><tr><th>NISN</th><th>Nama</th><th>JK</th></tr></thead>
      <tbody>
        <?php if (empty($widgets['kelas_diampu'])): ?>
          <tr><td colspan="3" class="text-center text-muted py-4">Belum ada kelas yang diampu.</td></tr>
        <?php else: foreach ($widgets['kelas_diampu'] as $s): ?>
          <tr><td><?= esc($s['nisn']) ?></td><td><?= esc($s['nama']) ?></td><td><?= esc($s['jenis_kelamin']) ?></td></tr>
        <?php endforeach; endif; ?>
      </tbody>
    </table>
  </div>
