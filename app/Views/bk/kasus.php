...
<div id="bkKasusApp" data-base-url="<?= esc(base_url()) ?>">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Data otomatis dibatasi sesuai scope user.</p>
        </div>
        <?php if (!empty($initial['can_manage'])): ?>
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalKasus">Tambah Kasus</button>
        <?php endif; ?>
    </div>
    ...
    <?php else: ?>
        <div class="card mb-4">
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        <select id="kasusKategori" class="form-select">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                ...
                <?php if (!empty($initial['can_manage'])): ?>
                    <a class="btn btn-sm btn-outline-primary" id="btnKasusExport" href="#">Export XLSX</a>
                <?php endif; ?>
            </div>
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    <thead><tr><th>Tanggal</th><th>Siswa</th><th>Pelanggaran</th><th>Kategori</th><th>Poin</th><th>Keterangan</th></tr></thead>
            ...
            <div class="card-footer d-flex justify-content-between">
                ...
                <div class="btn-group">
                    <button id="kasusPrev" class="btn btn-sm btn-outline-secondary">Sebelumnya</button>
                    <button id="kasusNext" class="btn btn-sm btn-outline-secondary">Berikutnya</button>
                </div>
            </div>
        </div>
        ...
        <?php if (($initial['scope'] ?? '') !== 'DIRI_SENDIRI'): ?>
            <div class="mt-3">
                <a href="<?= esc(base_url('bk/kasus/top')) ?>" class="btn btn-outline-secondary">Lihat Top 20 Poin</a>
            </div>
        <?php endif; ?>

        <?php if (!empty($initial['can_manage'])): ?>
        <div class="modal fade" id="modalKasus" tabindex="-1">
            <div class="modal-dialog">
                <form class="modal-content" id="formKasus">
                    <div class="modal-header"><h5 class="modal-title">Tambah Catatan Kasus</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_pelanggaran" class="form-select" required>
                                ...
                                <?php foreach (($initial['pelanggaran'] ?? []) as $p): ?>
                                    <option value="<?= (int) $p['id'] ?>"><?= esc($p['nama_pelanggaran'] . ' — ' . $p['kategori'] . ' (' . $p['poin'] . ')') ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
