...
?>
    <div class="col-6 col-lg-3">
        <div class="card h-100"><div class="card-body">
            <div class="d-flex align-items-center gap-2 mb-2"><i class="bx <?= esc($kpi['icon']) ?> fs-4"></i><span class="small text-muted"><?= esc($kpi['label']) ?></span></div>
            <div class="fs-3 fw-bold"><?= $kpi['value'] === null ? '—' : esc((string) $kpi['value']) ?></div>
        </div></div>
    </div>
<?php endforeach; ?>
</div>
...
<div class="card mb-4">
    ...
    <div class="card-body"><div class="row g-3">
        ...
        <div class="col-6 col-md-3">
            <a class="card h-100 text-decoration-none shadow-none border" href="<?= esc(base_url($action['url'])) ?>" <?= ($action['url'] ?? '') === 'ptsp' ? 'target="_blank" rel="noopener"' : '' ?>>
                <div class="card-body"><i class="bx <?= esc($action['icon']) ?> fs-3 mb-2"></i><div class="fw-semibold"><?= esc($action['label']) ?></div><div class="small text-muted mt-1"><?= esc($action['description']) ?></div></div>
            </a>
        </div>
        <?php endforeach; ?>
    </div></div>
...
<div class="row g-4">
    <div class="col-12 col-xl-6">
        <div class="card h-100">
            <div class="card-header d-flex justify-content-between align-items-center"><h5 class="mb-0">Layanan Terbaru</h5><?php if (!empty($widgets['access']['layanan'])): ?><a class="btn btn-sm btn-outline-primary" href="<?= esc(base_url('ptsp/layanan')) ?>">Lihat Semua</a><?php endif; ?></div>
            <div class="list-group list-group-flush">
                <?php foreach (($widgets['layanan_terbaru'] ?? []) as $row): ?>
                <div class="list-group-item py-3"><div class="d-flex justify-content-between gap-2"><div><div class="fw-semibold"><?= esc($row['nama_lengkap'] ?? '-') ?></div><div class="small text-muted"><?= esc(($row['jenis_layanan'] ?? '-') . ' · ' . ($row['kategori_pemohon'] ?? '-')) ?></div></div><span class="badge bg-label-primary"><?= esc($row['status'] ?? '-') ?></span></div></div>
                <?php endforeach; ?>
    ...
    <div class="col-12 col-xl-6">
        <div class="card h-100">
            <div class="card-header d-flex justify-content-between align-items-center"><h5 class="mb-0">Pengaduan Terbaru</h5><?php if (!empty($widgets['access']['pengaduan'])): ?><a class="btn btn-sm btn-outline-primary" href="<?= esc(base_url('ptsp/pengaduan')) ?>">Lihat Semua</a><?php endif; ?></div>
            <div class="list-group list-group-flush">
                <?php foreach (($widgets['pengaduan_terbaru'] ?? []) as $row): ?>
                <div class="list-group-item py-3"><div class="d-flex justify-content-between gap-2"><div><div class="fw-semibold"><?= esc($row['judul_laporan'] ?? '-') ?></div><div class="small text-muted"><?= esc($row['tanggal_kejadian'] ?: ($row['created_at'] ?? '-')) ?></div></div><span class="badge bg-label-warning"><?= esc($row['status'] ?? '-') ?></span></div></div>
                <?php endforeach; ?>
...
<div class="card mt-4">
    <div class="card-header d-flex justify-content-between align-items-center"><h5 class="mb-0">Ringkasan Kepuasan</h5><?php if (!empty($widgets['access']['polling'])): ?><a class="btn btn-sm btn-outline-primary" href="<?= esc(base_url('ptsp/polling')) ?>">Lihat Detail</a><?php endif; ?></div>
