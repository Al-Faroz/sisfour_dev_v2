...
class SettingsUser extends BaseController
{
    ...
    public function index()
    {
        if ($this->request->getGet('mode') === 'options') {
            return $this->respond(
                $this->service->identityOptions($this->request->getGet())
            );
        }
        ...
        if ($this->wantsJson()) {
            if ($this->request->getGet('id')) {
                return $this->respond(
                    $this->service->get((int) $this->request->getGet('id'))
                );
            }

            return $this->respond(
                $this->service->page($this->request->getGet())
            );
        }

        return $this->response->setBody(
            $this->renderWithLayout('settings/user', [
            ...
            ->setStatusCode(
                $success
                    ...
                    : match ($result['code'] ?? '') {
                        ...
                        'DUPLICATE', 'IDENTITY_IN_USE', 'LAST_ADMIN' => ResponseInterface::HTTP_CONFLICT,
