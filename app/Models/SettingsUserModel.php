...
class SettingsUserModel
{
    ...
    public function identityInUse(
        string $column,
        int $identityId,
        ?int $exceptUserId = null
    ): bool {
        $allowed = ['id_guru', 'id_pegawai', 'id_siswa'];
    ...
    public function getIdentityOptions(
        ...
        if ($type === 'guru') {
            $builder = $this->db
                ->table('guru g')
                ->select("g.id, g.nip AS kode, g.nama, u.id AS id_user", false)
        ...
        if ($type === 'pegawai') {
            $builder = $this->db
                ->table('pegawai p')
                ->select("p.id, p.nip AS kode, p.nama, u.id AS id_user", false)
    ...
    public function countActiveEffectiveAdmins(?int $exceptUserId = null): int
    {
        $sql = "
            ...
            FROM users u
            LEFT JOIN user_roles ur ON ur.id_user = u.id
    ...
    private function baseBuilder(array $filter)
    {
        ...
        if (!empty($filter['role'])) {
            ...
            $builder->groupStart()
                ->where('u.role', $role)
                ->orWhere(
                    "EXISTS (SELECT 1 FROM user_roles ur WHERE ur.id_user = u.id AND ur.role = " .
                    $this->db->escape($role) . ')',
                    null,
                    false
                )
